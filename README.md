...
```
sudo mkdir -p var/bot/BOTNAME;
sudo chown -R bot /var/bot/
cd var/bot/BOTNAME;
pip install -r requirements.txt;
python3 -m venv venv;
cp /var/bot/BOTNAME/bot_template.service /etc/systemd/system/
```
...
- **postgres database**
  ...
  - **upload source code via SFTP to `/var/bot/BOTNAME/`**
  
  ```
  cp -a /var/bot/BOTNAME/data/supporter_sources/country/country.csv /tmp;
  cp -a /var/bot/BOTNAME/data/supporter_sources/airport/airport.csv /tmp/;
  cp -a /var/bot/BOTNAME/data/database/bot.sql /tmp;  
  systemctl start postgresql.service;
  sudo -i -u postgres;  
  createdb bot; 
  psql -d bot -c "CREATE schema bot;";
  psql -d bot -c "SET schema 'bot';";
  psql -d bot -c "ALTER USER postgres PASSWORD 'YOUR_PASSWORD';";
  psql -h localhos -p 5432 -U postgres -d bot -a -q -f /tmp/bot.sql;
