...
async def _manage_commands(users: list | set, action: ItemAction, commands: list[dict[str, str]] = None):
    if users:
        for user_id in users:
            if commands:
                ...
                if action == ItemAction.SET:
                    await bot.set_my_commands(list(set_of_common_commands), scope)
                elif action == ItemAction.DELETE and scope:
                    await bot.delete_my_commands(scope)


async def manage_commands(action: ItemAction, users_id: int = None, command_list: list[dict[str, str]] = None):
    if users_id:
