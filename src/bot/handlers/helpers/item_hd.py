...
from bot.filters.command_filters import command_admin, command_manager
from bot.keyboards.staff.admin_kb import post_user_mgmt_message_IK_admin, itemManagerModel_admin, admin_panel_ADD_item_func, \
...
from bot.keyboards.staff.manager_kb import itemManagerModel_manager, manager_panel_ADD_item_func, \
    manager_panel_UPDATE_item_func, manager_panel_DELETE_item_func, post_user_mgmt_message_IK_manager
...
async def items_handler(callback_query: types.CallbackQuery, callback_data: dict):
    ...
    if action == 'add':
        ...
        if callback == command_admin.commands[0]:
            await post_user_mgmt_message_IK_admin(callback_query.from_user.id)
        if callback == command_manager.commands[0]:
            await post_user_mgmt_message_IK_manager(callback_query.from_user.id)
    if text:
...
async def update_item_hd(message: types.Message, state: FSMContext):
    if message.from_user.id in config.ADMINS:
        await itemManagerModel_admin.update_item(message, state, admin_panel_UPDATE_item_func)
    elif message.from_user.id in config.MANAGERS:
        await itemManagerModel_manager.update_item(message, state, manager_panel_UPDATE_item_func)


@dispatcher.message_handler(state=ItemMgmtStates.DELETE)
async def delete_item_hd(message: types.Message, state: FSMContext):
    if message.from_user.id in config.ADMINS:
        await itemManagerModel_admin.delete_item(message, state, admin_panel_DELETE_item_func)
    elif message.from_user.id in config.MANAGERS:
        await itemManagerModel_manager.delete_item(message, state, manager_panel_DELETE_item_func)
